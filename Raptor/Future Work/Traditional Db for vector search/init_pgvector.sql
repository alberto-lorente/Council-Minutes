...
CREATE TABLE chunks (
    doc_id              VARCHAR(80)     NOT NULL    REFERENCES doc_info(doc_id),     -- string of var len with up to 80 characters
...
CREATE TABLE generation_models (
    ...
    generation_model_name VARCHAR(20)   NOT NULL    -- name of the generation model 
);

-- Creating the table for the tables in the documents
CREATE TABLE tables (
    doc_id                  VARCHAR(80)     NOT NULL    REFERENCES doc_info(doc_id),   
    chunk_id                BIGINT          NOT NULL    REFERENCES chunks(chunk_id),   -- id of the chunk that contains the table
    ...
    generation_model_id     SERIAL		     REFERENCES generation_models(generation_model_id) -- id of the model used to generate the description
); --The generation model is being logged but not the embedding model. 
--The reason is the embedding is the same for all the chunks but there may be multiple generation models used for different reasons..
